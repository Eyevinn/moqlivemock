...
- **AV1 (`av01`) support.** `mlmpub` now serves AV1 video tracks in the CMSF
  namespaces (both CMAF and LOCMAF variants), including ClearKey/ECCP
  (`cenc`/`cbcs`) and commercial DRM protected variants. The decoder
  configuration is read from the `av1C` box via `mp4ff` (the new `AV1Data` codec
  type).
