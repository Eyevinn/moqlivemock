...
With `-cc608`, the publisher injects auto-generated CTA-608 CC1 captions into
the video elementary stream itself, so they travel inside the coded frames
...
| Codec | Carriage |
|-------|----------|
| AVC (`avc1`/`avc3`) | `user_data_registered_itu_t_t35` SEI NAL unit before the first VCL NALU |
| HEVC (`hvc1`/`hev1`) | prefix-SEI NAL unit before the first VCL NALU |
| AV1 (`av01`) | `metadata_itu_t_t35` OBU before the first frame OBU |

The `cc_data()` payload and its T.35/GA94 header are identical in all three;
only the envelope differs. Captions are generated by
[Eyevinn/go-608](https://github.com/Eyevinn/go-608).

When (and only when) `-cc608` is set, the MSF/CMSF catalogs advertise the
captions on each video track with an `accessibility` descriptor
(`urn:scte:dash:cc:cea-608:2015`, value `CC1=eng`). The catalogless `moq-mi`
namespace has nowhere to signal this, so its captions are in-band only.
